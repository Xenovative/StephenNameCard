...
if (inquiryFormElement && formNoteElement) {
  inquiryFormElement.addEventListener('submit', (submitEvent) => {
    submitEvent.preventDefault();

    const submittedForm = new FormData(inquiryFormElement);
    const submittedName = String(submittedForm.get('name') || '').trim();
    const submittedContact = String(submittedForm.get('contact') || '').trim();
    const submittedMessage = String(submittedForm.get('message') || '').trim();

    const mailSubject = encodeURIComponent(`Consultation Inquiry - ${submittedName || 'Website Visitor'}`);
    const mailBody = encodeURIComponent(
      `Name: ${submittedName}\nContact: ${submittedContact}\n\nRequirements:\n${submittedMessage || 'Not provided'}`
    );

    formNoteElement.textContent = 'Opening your email app...';
    window.location.href = `mailto:andyw@goldenthroneofficial.com?subject=${mailSubject}&body=${mailBody}`;
  });
}
